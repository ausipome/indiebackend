...
const helpOptions = (0, import_internals.format)(
  `${import_chalk.default.bold("Usage")}

${import_chalk.default.dim("$")} prisma db execute [options]
...
const _DbExecute = class {
  ...
  async parse(argv) {
    ...
    if (args["--preview-feature"]) {
      import_internals.logger.warn(`"prisma db execute" was in Preview and is now Generally Available.
...
See \`${import_chalk.default.green((0, import_internals.getCommandWithExecutor)("prisma db execute -h"))}\``
      );
...
DbExecute.help = (0, import_internals.format)(`
${process.platform === "win32" ? "" : import_chalk.default.bold("\u{1F4DD} ")}Execute native commands to your database

This command takes as input a datasource, using ${import_chalk.default.green(`--url`)} or ${import_chalk.default.green(
  `--schema`
)} and a script, using ${import_chalk.default.green(`--stdin`)} or ${import_chalk.default.green(`--file`)}.
